function varargout = jpg_to_dcm(varargin)
...
[~,folder_name] = uigetfile( ...
    {  '*.jpg;*.JPG;*.jpeg','JPG (*.jpg,*.JPG,*.jpeg)'; ...
    '*.*',  'All Files (*.*)'}, ...
    'Select a jpg', ...
    'MultiSelect', 'off',...
    dicompath);

set( handles.jpgFolder,'String',folder_name);
...
him = [];
for ii=1:numel(jpg_files)
    
    D = imread(fullfile(jpg_folder,jpg_files(ii).name));
    if ii == 1
        him = imshow(D);
    else
